pub use crate::pac::{Interrupt, INTERRUPT_CORE0, SYSTIMER};
pub fn get_time() -> u64 {
    unsafe {
...
pub fn set_target0_alarm_from_timestamp(timestamp: u64) {
    unsafe {
        ...
        systimer
            .target0_lo
            .write(|w| w.timer_target0_lo().bits((target_time & 0xFFFF_FFFF) as u32));
        //5. Set SYSTIMER_TIMER_COMPx_LOAD to synchronize the alarm value (t) to COMPx, i.e. load the alarm
        systimer.comp0_load.write(|w| w.timer_comp0_load().set_bit());
        //6. Set SYSTIMER_TARGETx_WORK_EN to enable the selected COMPx. COMPx starts comparing the count
        systimer
            .conf
            .modify(|_r, w| w.target0_work_en().set_bit().timer_unit0_core0_stall_en().clear_bit());
...
pub fn set_target1_alarm_from_timestamp(timestamp: u64) {
    unsafe {
        ...
        systimer
            .target1_lo
            .write(|w| w.timer_target1_lo().bits((target_time & 0xFFFF_FFFF) as u32));
        //5. Set SYSTIMER_TIMER_COMPx_LOAD to synchronize the alarm value (t) to COMPx, i.e. load the alarm
        systimer.comp1_load.write(|w| w.timer_comp1_load().set_bit());
        //6. Set SYSTIMER_TARGETx_WORK_EN to enable the selected COMPx. COMPx starts comparing the count
        systimer
            .conf
            .modify(|_, w| w.target1_work_en().set_bit().timer_unit0_core0_stall_en().clear_bit());
        // 7. Set SYSTIMER_TARGETx_INT_ENA to enable timer interrupt. When Unitn counts to the alarm value (t), a
        // SYSTIMER_TARGETx_INT interrupt is triggered
...
pub fn set_target2_alarm_from_timestamp(timestamp: u64) {
    unsafe {
        ...
        systimer
            .target2_lo
            .write(|w| w.timer_target2_lo().bits((target_time & 0xFFFF_FFFF) as u32));
        //5. Set SYSTIMER_TIMER_COMPx_LOAD to synchronize the alarm value (t) to COMPx, i.e. load the alarm
        systimer.comp2_load.write(|w| w.timer_comp2_load().set_bit());
        //6. Set SYSTIMER_TARGETx_WORK_EN to enable the selected COMPx. COMPx starts comparing the count
        systimer
            .conf
            .modify(|_, w| w.target2_work_en().set_bit().timer_unit0_core0_stall_en().clear_bit());
        // 7. Set SYSTIMER_TARGETx_INT_ENA to enable timer interrupt. When Unitn counts to the alarm value (t), a
        // SYSTIMER_TARGETx_INT interrupt is triggered
