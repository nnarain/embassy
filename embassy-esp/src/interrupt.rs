...
pub mod ESP32C3_Interrupts {
    ...
    pub fn set_kind(which: CpuInterrupt, kind: InterruptKind) {
        unsafe {
            ...
            intr.cpu_int_type.modify(|r, w| {
                w.bits(r.bits() & !(1 << cpu_interrupt_number) | (interrupt_type << cpu_interrupt_number))
            });
        }
    }
    ...
    pub fn clear(which: CpuInterrupt) {
        unsafe {
            let cpu_interrupt_number = which as isize;
            let intr = &*INTERRUPT_CORE0::ptr();
            intr.cpu_int_clear.write(|w| w.bits(1 << cpu_interrupt_number));
        }
    }
    ...
    pub fn get_priority(which: CpuInterrupt) -> u32 {
        unsafe {
            ...
            let x = intr_prio_base.offset(cpu_interrupt_number as isize).read_volatile();
            return x;
        }
    }
}

static SW_INT1_HANDLER: embassy_riscv::interrupt::Handler = embassy_riscv::interrupt::Handler::new();
...
unsafe impl Interrupt for SW_INT1 {
    ...
    unsafe fn __handler(&self) -> &'static embassy_riscv::interrupt::Handler {
        //TODO
...
impl InterruptExt for SW_INT1 {
    fn enable(&self) {
        unsafe {
            ...
            intr.cpu_int_enable.modify(|r, w| w.bits((1 << 4) | r.bits()));
...
unsafe impl ::embassy_hal_common::Unborrow for SW_INT1 {
    type Target = SW_INT1;
...
pub fn interrupt4(_: *mut TrapFrame) {
    unsafe {
        let func = SW_INT1_HANDLER.func.load(embassy::export::atomic::Ordering::Relaxed);
        let ctx = SW_INT1_HANDLER.ctx.load(embassy::export::atomic::Ordering::Relaxed);
